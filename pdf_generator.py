...
def create_pdf_header(pdf: FPDF, cleaned_cv_data, profile_picture_path: str):
    ...
    pdf.cell(LINE_WIDTH, HEADER_LINE_HEIGHT, os.getenv("NAME"), ln=True)  
...
def create_experience_section(pdf: FPDF, cleaned_updated_cv_data: dict):
    ...
    pdf.set_font(FONT, "", PARAGRAPH_FONT_SIZE)        # Regular font for content
    pdf.multi_cell(LINE_WIDTH, LINE_HEIGHT, cleaned_updated_cv_data.get("cv_job_experience", ""))  
...
def create_skills_section(pdf:FPDF, cleaned_updated_cv_data:dict):
    ...
    pdf.set_font(FONT, "", PARAGRAPH_FONT_SIZE)
    skills = cleaned_updated_cv_data["skills"]
