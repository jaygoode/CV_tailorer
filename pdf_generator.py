...
def generate_cv_pdf(
    updated_cv_data: dict, cv_data: dict, profile_picture_path: str, folder_path: Path
):
    """
...
def create_pdf_header(pdf: FPDF, cleaned_cv_data, profile_picture_path: str):
    ...
    pdf.cell(
        LINE_WIDTH, SUB_HEADER_LINE_HEIGHT, cleaned_cv_data["role"], ln=True
    )  # cell(width, height, text, ln=True moves cursor to next line)
...
def create_experience_section(pdf: FPDF, cleaned_updated_cv_data: dict):
    ...
    sections_experience_text = re.split(
...
def create_achievements_section(pdf, cleaned_updated_cv_data):
    ...
    pdf.set_font(FONT, "", PARAGRAPH_FONT_SIZE)
    pdf.multi_cell(
        LINE_WIDTH, LINE_HEIGHT, cleaned_updated_cv_data.get("key_achievements", "")
    )
